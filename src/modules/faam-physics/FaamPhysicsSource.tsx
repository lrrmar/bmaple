...
import FaamPhysicsLayer, {
  FaamPhysics,
  isPendingFaamPhysics,
} from './FaamPhysicsLayer';
import {
  useAppSelector as useSelector,
  useAppDispatch as useDispatch,
} from '../../hooks';
import { selectCache, request } from '../../mapping/cacheSlice';
import { selectLayerNames } from './faamPhysicsSlice';
import { v4 as uuidv4 } from 'uuid';

const FaamPhysicsSource = ({
  sourceIdentifier,
}: {
  ...
  useEffect(() => {
    ...
    const components = filteredIds.map((id) => (
      <FaamPhysicsLayer key={id} id={id} sourceIdentifier={sourceIdentifier} />
    ));
...
export default FaamPhysicsSource;
